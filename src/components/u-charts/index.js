...
const util = {
  ...
  isCollision: function isCollision(obj1, obj2) {
    ...
    const flag = obj2.start.x > obj1.end.x
...
function measureText(text) {
  const fontSize = arguments.length > 1 && arguments[1] !== undefined
    ? arguments[1]
    : config.fontSize;
  text = String(text);
  var text = text.split('');
...
function calLegendData(series, opts, config, chartData) {
  ...
  const { padding } = opts.legend;
  const { margin } = opts.legend;
  const { fontSize } = opts.legend;
...
function fixColumeData(points, eachSpacing, columnLen, index, config, opts) {
  return points.map((item) => {
    ...
    item.width = Math.ceil(
      (eachSpacing - 2 * config.columnPadding) / columnLen,
    );
...
function fixColumeMeterData(
  ...
  return points.map((item) => {
    ...
    item.width = Math.ceil((eachSpacing - 2 * config.columnPadding) / 2);
...
function getYAxisTextList(series, opts, config, stack) {
  ...
  const { minRange } = dataRange;
  const { maxRange } = dataRange;
...
function drawGaugeLabel(
  ...
  for (let i = 0; i < gaugeOption.splitLine.splitNumber + 1; i++) {
    ...
    const labelText = gaugeOption.labelFormat
...
function drawToolTipHorizentalLine(
  ...
  if (toolTipOption.yAxisLabel) {
    const labelText = calTooltipYAxisData(
...
function drawToolTip(
  ...
  const toolTipWidth = legendWidth
    + legendMarginRight
...
function drawYAxisTitle(title, opts, config, context) {
  const startX = config.xAxisHeight
...
function drawCandleDataPoints(series, seriesMA, opts, config, context) {
  ...
  series.forEach((eachSeries, seriesIndex) => {
    ...
    maxRange = ranges.shift();
    const { data } = eachSeries;
    const points = getCandleDataPoints(
      data,
      minRange,
      maxRange,
      xAxisPoints,
  ...
  if (opts.xAxis.disabled !== true) {
    ...
    if (config._xAxisTextAngle_ === 0) {
      ...
      newCategories.forEach((item, index) => {
        ...
        const _calRotateTranslate = calRotateTranslate(
          xAxisPoints[index] + eachSpacing / 2,
          startY + xAxisFontSize / 2 + 5,
          opts.height,
        );
        const { transX } = _calRotateTranslate;
        const { transY } = _calRotateTranslate;
...
function drawFunnelDataPoints(series, opts, config, context) {
  ...
  const { activeWidth } = funnelOption;
  const radius = Math.min(
...
function drawFunnelText(
  ...
  for (let i = 0; i < series.length; i++) {
    const item = series[i];
    let startX; let endX; let startY; let
      fontSize;
    const text = item.format
      ? item.format(+item._proportion_.toFixed(2))
      : `${util.toFixed(item._proportion_ * 100)}%`;
    ...
    case 'line':
      this.animationInstance = new Animation({
        ...
        onProcess: function onProcess(process) {
          ...
          const _drawLineDataPoints = drawLineDataPoints(
            series,
            opts,
            config,
            context,
    ...
    case 'mix':
      this.animationInstance = new Animation({
        ...
        onProcess: function onProcess(process) {
          ...
          drawYAxisGrid(categories, opts, config, context);
          drawXAxis(categories, opts, config, context);
          const _drawMixDataPoints = drawMixDataPoints(
            series,
            opts,
            config,
            context,
    ...
    case 'column':
      this.animationInstance = new Animation({
        ...
        onProcess: function onProcess(process) {
          ...
          drawYAxisGrid(categories, opts, config, context);
          drawXAxis(categories, opts, config, context);
          const _drawColumnDataPoints = drawColumnDataPoints(
            series,
            opts,
            config,
            context,
    ...
    case 'area':
      this.animationInstance = new Animation({
        ...
        onProcess: function onProcess(process) {
          ...
          drawYAxisGrid(categories, opts, config, context);
          drawXAxis(categories, opts, config, context);
          const _drawAreaDataPoints = drawAreaDataPoints(
            series,
            opts,
            config,
            context,
    ...
    case 'candle':
      this.animationInstance = new Animation({
        ...
        onProcess: function onProcess(process) {
          ...
          drawYAxisGrid(categories, opts, config, context);
          drawXAxis(categories, opts, config, context);
          const _drawCandleDataPoints = drawCandleDataPoints(
            series,
            seriesMA,
            opts,
            config,
...
const Charts = function Charts(opts) {
  ...
  config$$1.yAxisTitleWidth = opts.yAxis.disabled !== true && opts.yAxis.title
    ? config$$1.yAxisTitleWidth
    : 0;
  if (opts.type == 'pie' || opts.type == 'ring') {
    config$$1.pieChartLinePadding = opts.dataLabel === false
      ? 0
  ...
  config$$1.pieChartTextPadding = opts.dataLabel === false
    ? 0
  ...
  config$$1.columnPadding = config.columnPadding * opts.pixelRatio;
...
Charts.prototype.updateData = function () {
  ...
  switch (scrollPosition) {
    ...
    case 'left':
      ...
      break;
    case 'right':
      const _calYAxisData = calYAxisData(
        this.opts.series,
        this.opts,
        this.config,
      );
      ...
      let offsetLeft = 0;
      const _getXAxisPoints0 = getXAxisPoints(
...
Charts.prototype.zoom = function () {
  const val = arguments.length > 0 && arguments[0] !== undefined
    ? arguments[0]
...
Charts.prototype.showToolTip = function (e) {
  ...
  if (this.opts.type === 'mix') {
    ...
    if (index > -1) {
      ...
      var seriesData = getSeriesDataItem(this.opts.series, index);
      if (seriesData.length !== 0) {
        const _getMixToolTipData = getMixToolTipData(
          seriesData,
          this.opts.chartData.calPoints,
          index,
          this.opts.categories,
